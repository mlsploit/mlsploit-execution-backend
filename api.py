...
class ApiDataModel(object):
    ...
    def create(cls, **kwargs):
        ...
        r = RestClient.post(cls._endpoint, 
                            payload=kwargs, 
                            files=files)
    ...
    def get_all(cls, params=None):
        all_data = RestClient.get(
            cls._endpoint, params=params)
...
class Job(ApiDataModel):
    ...
    _expandable = {'owner': User, 'task': Task, 'run': Run,
                   'output_files': [File]}
    _json_props = ['output']
    ...
    def get_all_actionable(cls):
        all_pending_jobs = cls.get_all(
            params={'status': 'PENDING'})

        return list(filter(
            lambda j: j.parent_job is None or
                      j.parent_job.status == 'FINISHED',
            all_pending_jobs))
